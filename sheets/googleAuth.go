...
func GoogleAuth() *jwt.Config {
	...
	// fmt.Println("---------jsonCreds---------")
	// fmt.Println(jsonCreds.Private_key)
	// fmt.Println("---------jsonStr---------")
	// fmt.Println(jsonStr)
	// jsonEnv := os.Getenv("google_auth_json_string")
	// fmt.Printf("jsonEnv: %s \n", jsonEnv)
	byt := []byte(jsonStr)
	
	// byt := []byte(string(jsonEnv))
	// fmt.Printf("byt: %s \n", byt)
	...
	// }
	conf, err := google.JWTConfigFromJSON(byt, "https://www.googleapis.com/auth/spreadsheets")
	if err != nil {
		log.Fatal(err)
